...
- 数组访问与赋值
  ...
  数组和字符串的简单越界错误都会在**编译期**间发现，如直接用整数或者常量访问数组；

  但是**使用变量访问数组**编译器就无法提前发现错误，需要运行时阻止不合法的访问。

### 3.4 字符串

一片连续的内存空间，可以看做一个由字符串组成的数据，Go中的字符串是一个**只读**的字节数组。

字符串会被分配到只读的内存空间，不支持直接修改 string 类型的变量，所有在字符串上的写入操作都是通过拷贝实现的，修改字符串则通过 stirng 和 []byte类型之间转化实现。

string是可比较的，所以可以作为哈希的键

- 数据结构

  运行时的字符串用 `reflect.StringHeader` 表示
  ...
  与切片的数据结构很相似，所以字符串也可以被称为一个只读的切片类型
